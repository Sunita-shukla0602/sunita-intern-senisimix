...
class AlbertAttention(BertSelfAttention):
    def __init__(self, config):
        ...
        self.dense = q_Linear(config.hidden_size, config.hidden_size)
...
class AlbertLayer(nn.Module):
    def __init__(self, config):
        ...
        self.ffn = q_Linear(config.hidden_size, config.intermediate_size)
        self.ffn_output = q_Linear(config.intermediate_size, config.hidden_size)
...
class AlbertTransformer(nn.Module):
    def __init__(self, config):
        ...
        self.output_attentions = config.output_attentions
        self.output_hidden_states = config.output_hidden_states
        self.embedding_hidden_mapping_in = q_Linear(config.embedding_size, config.hidden_size)
...
class AlbertModel(AlbertPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.pooler = q_Linear(config.hidden_size, config.hidden_size)
...
class AlbertMLMHead(nn.Module):
    def __init__(self, config):
        ...
        self.dense = q_Linear(config.hidden_size, config.embedding_size)
        self.decoder = q_Linear(config.embedding_size, config.vocab_size)
        self.activation = ACT2FN[config.hidden_act]
...
class AlbertForSequenceClassification(AlbertPreTrainedModel):
    def __init__(self, config):
        ...
        self.classifier = q_Linear(config.hidden_size, self.config.num_labels)
...
class AlbertForQuestionAnswering(AlbertPreTrainedModel):
    def __init__(self, config):
        ...
        self.albert = AlbertModel(config)
        self.qa_outputs = q_Linear(config.hidden_size, config.num_labels)
