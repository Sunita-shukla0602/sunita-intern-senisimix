...
from src.models.quantized_modules import *
from src.models.configuration_mpqbert import MPQBertConfig
...
class MultiHeadSelfAttention(nn.Module):
    def __init__(self, config):
        ...
        self.q_lin = q_Linear(in_features=config.dim, out_features=config.dim)
        self.k_lin = q_Linear(in_features=config.dim, out_features=config.dim)
        self.v_lin = q_Linear(in_features=config.dim, out_features=config.dim)
        self.out_lin = q_Linear(in_features=config.dim, out_features=config.dim)
...
class FFN(nn.Module):
    def __init__(self, config):
        ...
        self.lin1 = q_Linear(in_features=config.dim, out_features=config.hidden_dim)
        self.lin2 = q_Linear(in_features=config.hidden_dim, out_features=config.dim)
...
class DistilBertForMaskedLM(DistilBertPreTrainedModel):
    def __init__(self, config):
        ...
        self.vocab_transform = q_Linear(config.dim, config.dim)
        self.vocab_layer_norm = nn.LayerNorm(config.dim, eps=1e-12)
        self.vocab_projector = q_Linear(config.dim, config.vocab_size)
...
class DistilBertForSequenceClassification(DistilBertPreTrainedModel):
    def __init__(self, config):
        ...
        self.distilbert = DistilBertModel(config)
        self.pre_classifier = q_Linear(config.dim, config.dim)
        self.classifier = q_Linear(config.dim, config.num_labels)
...
class DistilBertForQuestionAnswering(DistilBertPreTrainedModel):
    def __init__(self, config):
        ...
        self.distilbert = DistilBertModel(config)
        self.qa_outputs = q_Linear(config.dim, config.num_labels)
...
class DistilBertForTokenClassification(DistilBertPreTrainedModel):
    def __init__(self, config):
        ...
        self.distilbert = DistilBertModel(config)
        self.dropout = nn.Dropout(config.dropout)
        self.classifier = q_Linear(config.hidden_size, config.num_labels)
