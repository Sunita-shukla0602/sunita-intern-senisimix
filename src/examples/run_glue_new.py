...
from src.models.modeling_mpqdistilbert import DistilBertForSequenceClassification
from src.models.modeling_mpqalbert import AlbertForSequenceClassification
...
MODEL_CLASSES = {
    ...
    "mpqbert": (MPQBertConfig, MPQBertForSequenceClassification, BertTokenizer),
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                for i in range(args.num_layers - PT_, args.num_layers):
                    ...
                    output_mask = torch.where(model.bert.encoder.layer[i].output.dense.weight <= -1, torch.zeros_like(model.bert.encoder.layer[i].output.dense.weight), torch.ones_like(model.bert.encoder.layer[i].output.dense.weight))
                    
                    model.bert.encoder.layer[i].intermediate.dense.weight.grad = model.bert.encoder.layer[i].intermediate.dense.weight.grad + (model.bert.encoder.layer[i].intermediate.dense.quantized_weight.grad * intermediate_mask)
                    model.bert.encoder.layer[i].output.dense.weight.grad = model.bert.encoder.layer[i].output.dense.weight.grad + (model.bert.encoder.layer[i].output.dense.quantized_weight.grad * output_mask)
...
def evaluate(args, model, tokenizer, prefix=""):
    ...
    for eval_task, eval_output_dir in zip(eval_task_names, eval_outputs_dirs):
        ...
        with open(output_eval_file, "a+") as writer:  # a means append, w overwrite  
...
def main():
    ...
    if args.save_quantized_model:
        ...
        for i, j in list(state_dict_quant_mpq.items()):
            if 'pooler' in i or 'LayerNorm' in i or 'bias' in i or 'q_min' in i or 'q_max' in i:
                state_dict_quant_mpq[i] = j.half()
            if 'bert.embeddings.word_embeddings.weight' in i or 'bert.embeddings.position_embeddings.weight' in i:
                state_dict_quant_mpq[i] = quantization(j,8).to(dtype=torch.int8)
            if 'token_type_embeddings' in i:
                state_dict_quant_mpq[i] = j.half()
            if 'classifier' in i:
                state_dict_quant_mpq[i] = j.half()

            for k in range(config.num_8bit_layers):
                if ('query.weight' in i or 'key.weight' in i or 'value.weight' in i or 'dense.weight' in i) and ('encoder' in i and str(k) in i):
                    print(i)
                    state_dict_quant_mpq[i] = quantization(j,8).to(dtype=torch.int8)
            for q in range(config.num_8bit_layers, config.num_hidden_layers):
                if ('query.weight' in i or 'key.weight' in i or 'value.weight' in i or 'attention.output.dense.weight' in i) and ('encoder' in i and str(q) in i):
                    state_dict_quant_mpq[i] = quantization(j,8).to(dtype=torch.int8)
                if (".intermediate.dense.weight" in i or ".output.dense.weight" in i and "attention" not in i) and ("encoder" in i and str(q) in i):
                    del state_dict_quant_mpq[i]
        ...
        if not os.path.exists(args.quantized_model_dir):
            os.makedirs(args.quantized_model_dir)
        
        model_to_save.save_pretrained(args.quantized_model_dir) # Save the config.json file
        tokenizer.save_pretrained(args.quantized_model_dir) # Save the vocab.txt file

        torch.save(state_dict_quant_mpq, args.quantized_model_dir + "/pytorch_model.bin") # Save the quantized model
