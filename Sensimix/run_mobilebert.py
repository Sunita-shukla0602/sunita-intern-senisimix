...
for task, data_dir in datasets:
    print(f"==========================================")
    print(f"Running: MobileBERT on {task}")
    print(f"==========================================")

    # Start GPU logger
    gpu_log = f"mobilebert_{task}_gpu_log.csv"
    ...
    cmd = [
        ...
        "--learning_rate", "2e-5",
        "--num_train_epochs", "3",
        "--output_dir", f"outputs/mobilebert_{task}",
        "--quantized_model_dir", f"quantized_models/mobilebert_{task}",
    ...
    print(f"Done: MobileBERT on {task}")

print("All MobileBERT datasets completed.")
