...
for task, data_dir in datasets:
    print(f"==========================================")
    print(f"Running: TinyBERT on {task}")
    print(f"==========================================")

    # Start GPU logger
    gpu_log = f"tinybert_{task}_gpu_log.csv"
    ...
    cmd = [
        ...
        "--learning_rate", "1e-5",
        "--num_train_epochs", "3",
        "--output_dir", f"outputs/tinybert_{task}",
        "--quantized_model_dir", f"quantized_models/tinybert_{task}",
    ...
    print(f"Done: TinyBERT on {task}")

print("All TinyBERT datasets completed.")
